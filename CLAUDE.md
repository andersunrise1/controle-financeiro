...
Verificado de ponta a ponta, de verdade: depois do fix de porta, `POST /api/auth/login` responde 400 corretamente contra a URL de produção; o usuário se cadastrou com a conta real (`andersunrise1@gmail.com`) direto pelo app já instalado no celular, sem o notebook precisar estar ligado durante o teste, e confirmou funcionando. Ainda não verificado: um redeploy de verdade **depois** de já existirem dados reais no volume (o único redeploy até agora aconteceu antes da conta ser criada) — a lógica de persistência está certa (mesmo path usado localmente), mas só um redeploy com dados reais já no banco confirma na prática.
