...
6. **Testes reais** — ~1-2 dias — **parte de código concluída em 2026-08-20; falta o teste no celular de verdade, ação do usuário**. `ajustes de teclado`: nenhuma tela com campo de texto usava `KeyboardAvoidingView` fora de Login/Cadastro (que já tinham desde a Etapa 1) — `DashboardScreen.jsx` (onde fica o formulário de Nova Transação, bem no meio de uma tela longa e rolável) e `FeedbackScreen.jsx` não tinham, então em iOS o teclado podia cobrir o campo em foco sem nenhum ajuste automático (Android já tem `adjustResize` nativo). Corrigido com o mesmo padrão já usado no Login (`behavior="padding"` só no iOS, `ScrollView` com `keyboardShouldPersistTaps="handled"`). `área segura`: já estava correta desde as etapas anteriores (`SafeAreaView` em toda tela, `edges={["top"]}` nas telas dentro das abas — a área de baixo já fica coberta pela própria barra de abas).

   **Bug real achado nesse processo, não relacionado a teclado**: o IP de rede local gravado no `eas.json` (`192.168.0.65`, usado nos perfis `development`/`preview` do EAS Build pra apontar `EXPO_PUBLIC_API_URL`) estava desatualizado — o `ipconfig` desse computador mostra `192.168.0.75` agora (o roteador reatribuiu o IP em algum momento desde que aquele valor foi gravado). Um teste real no celular com o IP antigo falharia silenciosamente (erro de conexão recusada, sem pista óbvia da causa). Corrigido no `eas.json` e documentado num `mobile/.env.example` novo — pra rodar via `npx expo start` (não EAS Build), o app lê `EXPO_PUBLIC_API_URL` de um `mobile/.env` (gitignored, criado com o IP atual). Adicionado `npm run start:tunnel` (`expo start --tunnel`) como alternativa que não depende de celular e computador estarem na mesma rede Wi-Fi.

   Verificado o que dava pra verificar sem celular: registrei uma conta de teste pela preview mobile-web, confirmei visualmente que `DashboardScreen` (rolando até o formulário, focando e digitando no campo Descrição) e `FeedbackScreen` renderizam e recebem texto normalmente com o novo `KeyboardAvoidingView` — sem quebra de layout, sem erro no console. **O que só dá pra confirmar com um celular de verdade**: o comportamento real do teclado nativo (o preview do navegador não abre teclado virtual de verdade), e o fluxo de conexão via Expo Go (`npm run start`, escanear o QR code, mesmo Wi-Fi do computador com `EXPO_PUBLIC_API_URL` apontando pro IP certo — ou `npm run start:tunnel` se não estiverem na mesma rede). Esse ambiente não tem emulador nem celular físico — mesma limitação já registrada nas Etapas 1 e 5, e no app desktop do projeto irmão.
